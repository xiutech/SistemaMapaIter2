...
/**
 * Clase para el filtro de páginas para sesion de Comentarista.
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        HttpSession session = req.getSession(true);
         if (session.getAttribute("user") == null) {
            ...
            if(usuario.getRol()== SessionController.Rol.COMENTARISTA){
                chain.doFilter(req, res); // esta logueado un comentarista se continua con lo que se solicito.
            }else
                res.sendRedirect(req.getContextPath() + "/index.xhtml"); // Si no es comentarista, redirige al index
        }
    }
