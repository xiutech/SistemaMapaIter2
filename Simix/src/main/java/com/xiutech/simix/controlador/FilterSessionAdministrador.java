...
/**
 * Clase para el filtro de páginas para sesion de Admministrador.
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        HttpSession session = req.getSession(true);
         if (session.getAttribute("user") == null) {
            ...
            if(usuario.getRol()== SessionController.Rol.ADMINISTRADOR){
                chain.doFilter(req, res); // esta logueado un admin se continua con lo que se solicito.
            }else 
                res.sendRedirect(req.getContextPath() + "/index.xhtml"); // Si no es admin, redirige al index
        }
    }
